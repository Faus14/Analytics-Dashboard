...
export function NetworkStatsCard() {
  ...
  const stats = currentTick > 0 ? {
    currentTick,
    epoch,
    tickDuration: 0, // Real duration not available from context
      ...
      {isLoading && !stats ? (
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div className="bg-white/5 border border-white/10 rounded-lg p-4 hover:bg-white/8 transition-all">
            ...
            <p className="text-white text-lg font-bold">{stats.tickDuration}ms</p>
            <p className="text-white/40 text-xs mt-1">Block time</p>
          </div>
          ...
          <div className="bg-white/5 border border-white/10 rounded-lg p-4 hover:bg-white/8 transition-all">
            <div className="flex items-center gap-2 mb-2">
              ...
              <p className="text-white/60 text-sm">Qubic Age</p>
            </div>
            <p className="text-white text-lg font-bold">{stats.qubicAge}</p>
            <p className="text-white/40 text-xs mt-1">Network uptime</p>
          </div>
        </div>
      ) : null}
